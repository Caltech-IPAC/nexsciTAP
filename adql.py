...
#         and func2 == 'polygon' and nargs2 >= 7):
        ...
        # Point in box
         ...
         and func2 == 'box'):
            ...
            if self.debug:
                logging.debug('xxx3-1: call _box')

            box = self._box(lon, lat, width, height)

            if self.debug:
                logging.debug('xxx3-2: returned _box')
            ...
            if self.debug:
                logging.debug('xxx3-3: call spt.polygon_search')
                logging.debug(f'self.mode= {self.mode:d}')
                logging.debug(f'self.level= {self.level:d}')
                logging.debug(f'self.colname= {self.colname:s}')
                logging.debug(f'self.encoding= {self.encoding:d}')

            retval = spt.polygon_search(npts, ra, dec, self.mode, self.level, self.xcol, self.ycol, self.zcol, self.colname, self.encoding)

            if self.debug:
                logging.debug('xxx3-4: returned spt.polygon_search')
            ...
            if self.debug:
                logging.debug('xxx3-3: returned _box')


        else:
            if self.debug:
                logging.debug('xxx3-4: returned _box')

            raise Exception('Invalid CONTAINS() clause')

            if self.debug:
                logging.debug('xxx3-5: returned _box')
            ...
            if self.debug:
                logging.debug('xxx3-5-0: returned _box')
	    ...
	    for i in range(len(args1)):
                ...
                geomstr = geomstr + args1[i]
    
            if self.debug:
                logging.debug('xxx3-5-1: returned _box')
            
            geomstr = geomstr + '), '
    
            geomstr = geomstr + func2 + '('
            ...
            for i in range(len(args2)):
                ...
                geomstr = geomstr + args2[i]
    
            geomstr = geomstr + ')) = ' + val
            
            if self.debug:
                logging.debug('xxx3-5-2: returned _box')
            
            '''
           
            if self.debug:
                logging.debug('xxx3-6: returned _box')
